...
const CATEGORY_ICONS = {
  all: BookOpen,
  verstehen: GraduationCap,
  bitten: Clock,
  gesundheit: HeartPulse,
  material: Backpack,
  alltag: UtensilsCrossed,
  custom: Sparkles,
};
...
export default function SchoolPhrasesView({ onTransferToTranslator }) {
  ...
  const handleSpeakGerman = (phrase) => {
    setPlayingId(`${phrase.id}_de`);
    speechService.speak({
      text: phrase.de,
      lang: 'de-DE',
  ...
  const handleSpeakNative = (phrase) => {
    const text = getPhraseText(phrase, targetLang);
    setPlayingId(`${phrase.id}_native`);
    speechService.speak({
      text,
      lang: targetLang,
      onEnd: () => setPlayingId(null),
