...
export default function SettingsView({ isForcedOffline, onToggleForceOffline, onOpenOnboarding }) {
  ...
  const handleInstallLanguage = async (langItem) => {
    if (typeof navigator !== 'undefined' && !navigator.onLine) {
      alert('Für die Installation eines neuen Sprachpakets wird eine aktive Internetverbindung benötigt, um die Schul-Redemittel herunterzuladen.');
      return;
    }

    setIsInstalling(true);
    setInstallStatusMsg(null);
    const totalItems = SCHOOL_PHRASES.length;
    ...
    try {
      ...
      storageService.saveInstalledLanguage({
        ...langItem,
        description: 'Offline-Paket (Schüler)',
      ...
      setInstallStatusMsg({
        success: true,
        text: `Sprachpaket ${langItem.flag} ${langItem.name} erfolgreich installiert! Alle Redemittel stehen nun auch offline zur Verfügung.`
      });
    } catch (err) {
      setInstallStatusMsg({
        success: false,
